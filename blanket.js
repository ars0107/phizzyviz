...
d3.json('austin_2020.json').then(function(data) {
    console.log(data)
    ...
    function makeSquare(d){
        ...
        // middlest circle (max)
        outputString += `<circle cx=${0.5*edge} cy=${0.5*edge} r=${0.5*edge*2/3} fill=${color(d.TMAX)}></circle>`
        // smallest circle (min)
        outputString += `<circle cx=${0.5*edge} cy=${0.5*edge} r=${0.5*edge*1/3} fill=${color(d.TMIN)}></circle>`
